            ...
            for pull in pulls:
                for branch in branches:
                    ...
                    if branch in pull.head.label:
                        print(
                            "pull request #%s with from=%s in repo %s"
                            % (pull.number, branch, fullreponame)
                        )
...
def oldprocess_pr(repo_config, gh, repo, issue, dryRun, cmsbuild_user=None, force=False):
    ...
    mu2e_commenters = list(
        set(mu2e_write_mems)
    )

    print (CMSSW_L2)

    L1_CATS = CMSSW_L2["Mu2e/write"]
    ...
    new_tests = True
    if "CMS_BOT_MULTI_PR_TESTS" in environ:
        new_tests = True
    ...
    REGEX_EX_CMDS = (
        "^type\s+(bug(-fix|fix|)|(new-|)feature)|urgent|backport\s+(of\s+|)(#|http(s|):/+github\.com/+%s/+pull/+)\d+$"
        % (repo.full_name)
    )
    known_ignore_tests = "build-warnings|clang-warnings|none"
    known_enable_tests = "gpu|none"
    REGEX_EX_IGNORE_CHKS = "^ignore\s+(%s)(\s*,\s*(%s))*$" % (
        known_ignore_tests,
        known_ignore_tests,
    )
    REGEX_EX_ENABLE_TESTS = "^enable\s+(%s)(\s*,\s*(%s))*$" % (
        known_enable_tests,
        known_enable_tests,
    )
    ...
    if issue.pull_request:
        ...
        if repository == CMSDIST_REPO_NAME:
            ...
            if br:
                extra_rm = extra_rm + RELEASE_MANAGERS.get(br, [])
    ...
    for comment in all_comments:
        ...
        if (commenter in COMMENT_CONVERSION) and (
            comment.created_at <= COMMENT_CONVERSION[commenter]["comments_before"]
        ):
            orig_msg = comment_msg
            for cmt in COMMENT_CONVERSION[commenter]["comments"]:
                comment_msg = comment_msg.replace(cmt[0], cmt[1])
            if orig_msg != comment_msg:
                print(
                    "==>Updated Comment:",
                    commenter,
                    comment.created_at,
        ...
        if (commenter == cmsbuild_user) and re.match(ISSUE_SEEN_MSG, first_line):
            ...
            if create_external_issue:
                external_issue_number = comment_msg.split(
                    "external issue " + CMSDIST_REPO_NAME + "#", 2
                )[-1].split("\n")[0]
                if not re.match("^[1-9][0-9]*$", external_issue_number):
                    print(
                        "ERROR: Unknown external issue PR format:", external_issue_number
                    )
                    external_issue_number = ""
            continue
        ...
        if new_cats:
            ...
            if (
                (commenter in larsoft_l2_mems)
                or (commenter in mu2e_write_mems)
                or (commenter in CMSSW_ISSUES_TRACKERS + larsoft_l1_mems)
            ):
        ...
        if issue.pull_request or push_test_issue:
            ...
            # if (commenter in releaseManagers + larsoft_l1_mems) and re.match(
            #     "^\s*(merge)\s*$", first_line, re.I
            # ):
            #     mustMerge = True
            #     mustClose = False
            #     if (commenter in larsoft_l1_mems) and ("L1" in signatures):
            #         signatures["L1"] = "approved"
            #     continue
                ...
                if trigger_search is not None:
                    ...
                    if tests_already_queued:
                        print(
                            ...
                            comment.created_at - last_test_start_time,
                        )
                        diff = time.mktime(
                    ...
                    if not tests_already_queued:
                        ...
                        print("Tests already requested for this PR")
                    continue
                    ...
                    or (
                        (signatures["tests"] != "pending")
                        and (not comparison_done)
                        and (not push_test_issue)
                    )
                ):
                    tests_already_queued = False
                    abort_test = True
        ...
        #     commenter in larsoft_l2_mems
        #     or commenter in mu2e_write_mems
        #     and [x for x in CMSSW_L2[commenter] if x in signing_categories]
        # ):
        #     ctype = ""
        #     selected_cats = []
    ...
    if create_test_property:
        ...
        if tests_requested:
            ...
            for p in [x for x in cmsdist_pr.replace(" ", "").split(",") if x]:
                if "#" not in p:
                    p = "%s/cmsdist#%s" % (repo_org, p)
                prs.append(p)
            for p in [x for x in cmssw_prs.replace(" ", "").split(",") if x]:
                if "#" not in p:
                    p = "%s/Offline#%s" % (repo_org, p)
                if repo_org not in p:
                    p = "%s/%s" % (repo_org, p)
                prs.append(p)
            for xpr in prs:
                ...
                ex_msg = ""
                if rest_pr:
                    ex_msg = "\nTested with other pull request(s) %s" % ",".join(
                        rest_pr
                    )
                if not repo_name in repo_cache:
                    repo_cache[repo_name] = gh.get_repo(repo_name)
                pr_issue = issue
                if (repo_name != repository) or (pr_num != prId):
                    pr_issue = repo_cache[repo_name].get_issue(pr_num)
                if not dryRun:
                    ...
                    print(
                        "DryRun: Creating comment:%s\n\n%s"
                        % (xpr, TRIGERING_TESTS_MSG + ex_msg)
                    )
            if not dryRun:
                extra_prop = {}
                if release_queue:
                    extra_prop["RELEASE_FORMAT"] = release_queue
                if release_arch:
                    extra_prop["ARCHITECTURE_FILTER"] = release_arch
                if ignore_tests:
                    extra_prop["IGNORE_BOT_TESTS"] = ",".join(ignore_tests)
                if enabled_tests:
                    extra_prop["ENABLE_BOT_TESTS"] = ",".join(enabled_tests)
                if extra_wfs:
                    extra_prop["MATRIX_EXTRAS"] = extra_wfs
                create_properties_file_tests(
                    repository,
                    prId,
                    cmsdist_pr,
    ...
    missing_notifications = [
        ...
        and signature not in ["L1"]
        and name not in set(larsoft_l1_mems + larsoft_l2_mems + mu2e_write_mems)
    ]
    ...
    if watchers:
        watchersMsg = format(
            "%(watchers)s, this is something you requested to watch.\n",
    ...
    messageNewPR = format(
        ...
        " Thanks.\n"
        "%(watchers)s"
        'FNALbuild commands are listed <a href="https://mu2ewiki.fnal.gov/wiki/GitHubWorkflow">here</a>\n',
    ...
    messageUpdatedPR = format(
        "Pull request #%(pr)s was updated." "%(cmsdist_issue)s\n",
        pr=pr.number,
        cmsdist_issue='0'#cmsdist_issue,
    )
    ...
    commentMsg = ""
    print("Status: Already Seen = %s, Updated: %s" % (already_seen, pull_request_updated))
    ...
    if (pr.base.ref in RELEASE_BRANCH_CLOSED) and (pr.state != "closed"):
        ...
        if (
            (not triggerred_code_checks)
            and cmssw_repo
            and (pr.base.ref == "master")
            and ("code checks" in signatures)
            and (signatures["code checks"] == "pending")
        ):
            trigger_code_checks = True
    elif new_categories:
        commentMsg = messageUpdatedPR
    ...
    if trigger_code_checks and not triggerred_code_checks:
        if not dryRunOrig:
            ...
            print("Dryrun:", TRIGERING_CODE_CHECK_MSG)

        create_properties_file_tests(
            repository, prId, "", "", dryRunOrig, abort=False, req_type="codechecks"
        )
