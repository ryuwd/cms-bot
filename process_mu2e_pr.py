...
FAILED_TESTS_MSG = 'The Jenkins tests have failed, please try again.'
...
# all default tests
TEST_REGEXP_MU2E_DEFTEST_TRIGGER = "(@%s)(\s*[,:;]*\s+|\s+)(please\s*[,]*\s+|)(run\s+test(s|)|test)" % MU2E_BOT_USER
REGEX_DEFTEST_MU2E_PR = re.compile(TEST_REGEXP_MU2E_DEFTEST_TRIGGER, re.I)

# build test
TEST_REGEXP_MU2E_BUILDTEST_TRIGGER = "(@%s)(\s*[,:;]*\s+|\s+)(please\s*[,]*\s+|)(run\s+(build\s*)test(s|)|(test\s+(build)))" % MU2E_BOT_USER
REGEX_BUILDTEST_MU2E_PR = re.compile(TEST_REGEXP_MU2E_BUILDTEST_TRIGGER, re.I)

# code test
TEST_REGEXP_MU2E_LINTTEST_TRIGGER = "(@%s)(\s*[,:;]*\s+|\s+)(please\s*[,]*\s+|)(run\s+(code\s*)(test(s|)|check(s)|lint(er|ing))|(test\s+code|check\s+code))" % MU2E_BOT_USER
REGEX_LINTTEST_MU2E_PR = re.compile(TEST_REGEXP_MU2E_LINTTEST_TRIGGER, re.I)

# full physics validation
TEST_REGEXP_MU2E_VALIDATION_TRIGGER = "(@%s)(\s*[,:;]*\s+|\s+)(please\s*[,]*\s+|)(run\s+(full\s+|)(V|v)alidation)"
